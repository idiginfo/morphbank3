...
function fixImageFiles($id, $fileName, $imageType = null, $problems = null, $fileSourceDir = null, $width = null, $height = null) {
    ...
    if ($imageType == "jpg")
        $imageType = "jpeg"; // jpg original stored in orig

    $originalImgPath = getOriginalImageFilePath($id, $imageType);
    ...
    if (!empty ($fileImageType)) {
        ...
        // implement changed policy for location of jpeg original
        if ($imageType = "jpeg" && !file_exists(!$originalImgPath) && file_exists($jpegImgPath)) {
            //move original to orig if it is in jpeg
            rename($jpegImgPath, $originalImgPath);
        }
        ...
        //remove raw original for dng
        if ($imageType == 'dng' && strpos($originalImgPath, 'raw') != false) {
