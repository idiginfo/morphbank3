...
class mirrorFilter extends filter {
	...
	// Contructor
	function mirrorFilter( $myDomain = "http://morphbank2.scs.fsu.edu/") {		
	...
	function resetData() {
		...
		$this->actionUrl = $this->domainName.'Browse/ByMirror/index.php?pop=YES';
	...
	function display() {	
		...
		echo '	<tr>
					<td><input id="keywords" type="text" name="mirrorKeywords" value="'.$htmlValue.'" size="'.$this->size.'" 
	...
	function getSqlWhereContribution() {
		...
		if ( is_numeric($this->searchString) ) {
			...
			$sql .= '(Publication.id ='.$this->searchString.') ';
			$this->isTheFirst = FALSE;
			return $sql;
		}
		...
		elseif ($this->searchString != '') {
			...
			for ($i = 0; $i < $num; $i++) {
				$sql .= $this->isTheFirst?'WHERE ':'AND ';
				$sql .= '(ServerInfo.url LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'ServerInfo.serverId LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'ServerInfo.imageUrl LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'ServerInfo.locality LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'ServerInfo.tsns LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'Groups.groupName LIKE \'%'.$arrayOfWords[$i].'%\' OR '
					.'ServerInfo.dateCreated LIKE \'%'.$arrayOfWords[$i].'%\') ';
				$this->isTheFirst = FALSE;		
			}
			return $sql;			
		}
	...
	function retrieveDataFromGET() {
		...
		$this->setSearchString($_GET['mirrorKeywords']);
