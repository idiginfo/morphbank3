...
unique identifiers, use our options <a href="<?php echo $config->domain; ?>docs/mb3a.xls">Data Entry Workbook - Animalia</a> or <a href="<?php echo $config->domain; ?>docs/mb3p.xls">Data Entry Workbook - Plantae.</a> Note that these workbooks are located for easy download 
from <a href="<?php echo $config->domain; ?>Help/Documents/"> Documents</a> and are easily modified to suit other kingdoms.</p>
...
<div class="specialtext2">Custom Workbook - SAMPLE DATA SETS
<ul>
<li><a href="<?php echo $config->domain; ?>docs/scamit.xls">SCAMIT</a>: A data set from an invertebrate taxonomy group in Southern California. Note only fields they use are present. Also, drop-downs
are customized for the taxonomic group of interest as needed for different labs.</li>
<li><a href="<?php echo $config->domain; ?>docs/usms.xls">USMS</a>: From the herbarium at the University of Southern Mississipi, this data set was generated by mapping an existing database to 
Morphbank fields. An SQL query on that database generated this dataset with column headers that match Morphbank fields. Again note minimal fields used
to fit the needs / desires of this institution.</li>
<li><a href="<?php echo $config->domain; ?>docs/talltimbers.xls">Tall Timbers</a>: Similar to the above set, a research station interested in creating an image repostitory of the various specimen
collections created and housed at their facility, used an abbreviated version of the custom workbook. Note their data entry form shown here
 is tailored to show only the fields they need to work with.</li>
</ul>
</div>
...
<li>Taxon Names cannot be added with this workbook at this time. Users must add names via one of 3 possiblities:
  ...
  <ol>
    ...
    <li>If many names, at <em>rank Genus or lower</em> need adding to Morphbank, please use the <strong>Specimen Taxon Data worksheet</strong> in the <a href="<?php echo $config->domain; ?>docs/mb3a.xls">Data Entry Workbook - Animalia</a> or <a href="<?php echo $config->domain; ?>docs/mb3p.xls">Data Entry Workbook - Plantae.</a></li>
...
Suggestion: download and open the <a href="<?php echo $config->domain; ?>docs/customWorkbook.xls">Morphbank Custom Workbook</a> to look at while reading this documentation.</div>
...
<ol>
    ...
    <br />
	<ol>
	<li>Names can be added via the web-interface from Sub-Order and lower ranks. Go to HeaderMenu > Tools > Submit > Taxon 	Name to 	start. Try the <a href="<?php echo $config->domain; ?>About/Manual/addTaxonName.php" target="_blank">Online User Manual - Add Taxon Name</a> for help with this process.	</li>
    ...
    <li>Using <a href="<?php echo $config->domain; ?>Help/nameMatch">Name Query</a>, a list of taxon names can be searched all-at-once and a list of tsns returned. This list can be pasted directly into the <strong>Data sheet </strong><em>Determination TSN</em> column.</li>
</ol>
<br />
<br />
...
<li>In addition to the <strong>Field Help</strong> sheet, see the Morphbank Online User Manual for Morphbank Field descriptions for more about what type of data goes into which field and what the format of the data needs to be for each of those fields.
    ...
    <br />
    <ul>
	<li><a href="<?php echo $config->domain; ?>About/Manual/uploadSubmitImage.php" target="_blank">Image Fields</a></li>
	<li><a href="<?php echo $config->domain; ?>About/Manual/uploadSubmitSpecimen.php" target="_blank">Specimen Fields</a></li>
    <li><a href="<?php echo $config->domain; ?>About/Manual/uploadSubmitView.php" target="_blank">View Fields</a></li>
    <li><a href="<?php echo $config->domain; ?>About/Manual/uploadSubmitLocality.php" target="_blank">Locality Fields</a></li>
	</ul>
    <br />
...
<br />
<li>In addition to the <strong>Field Help</strong> sheet, see the <a href="<?php echo $config->domain; ?>docs/mbTablesDescription.pdf"> MB Table Descriptions</a> for even more about Morphbank database tables and field types  (varchar, text, field length restrictions).</li>
<br />
...
<li>The file will be validated to check the contents. This validation looks at field content match (text fields contain text, numeric fields contain numerics) and overall data. Morphbank will return the file to the sender with questions if need be, otherwise it will be uploaded. Always keep a copy of the workbook.</li>
			...
			<div id=footerRibbon></div>
			<table align="right">
<td><a href="<?php echo $config->domain; ?>About/Manual/UploadService.php" class="button smallButton"><div>Next</div></a></td>
