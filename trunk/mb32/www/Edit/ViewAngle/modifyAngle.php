...
<?php
/**
 File name: modifyAngle.php
 @author Neelima Jammigumpula <jammigum@scs.fsu.edu>
 @package Morphbank2
 @subpackage Submit
 @subpackage Edit
 @subpackage ViewAngle
 This script updates ViewAngle reference table with
...
$message = "Table:  View Angle <br /><br />";

//Get all the post variables for all the records as array element.
for ($i = 0; $i < $rows; $i++) {
	$name[$i] = mysqli_real_escape_string($link, trim($_POST[Angle . $i]));
	$description[$i] = mysqli_real_escape_string($link, trim($_POST[Description . $i]));
	$id[$i] = trim($_POST[id . $i]);

	$curr = mysqli_fetch_array(runQuery('SELECT * FROM ViewAngle WHERE name =\'' . $id[$i] . '\''));

	if ($curr['name'] != $name[$i]) {
		$modifiedFrom[$i] = 'View Angle: ' . $curr['name'] . ' ';
		$modifiedTo[$i] = 'View Angle: ' . $name[$i] . ' ';
		$flag = true;
	}
	...
	if (($curr['name'] != $name[$i]) && ($curr['description'] != $description[$i])) {
		$query[$i] = 'UPDATE ViewAngle SET  name = \'' . $name[$i] . '\', description = \'' . $description[$i] . '\' where name = \'' . $id[$i] . '\' ;';
		$message .= 'Record:  ' . $id[$i] . '<br />Field Name:  View Angle <br />Old Value:  ' . $curr['name'] . '<br />New Value:  ' . $name[$i] . '<br />' . 'Field Name:  Description <br />Old Value:  ' . $curr['description'] . '<br />New Value:  ' . $description[$i] . '<br />';
	} elseif ($curr['name'] != $name[$i]) {
		$query[$i] = 'UPDATE ViewAngle SET  name = \'' . $name[$i] . '\' where name = \'' . $id[$i] . '\' ;';
		$message .= 'Record:  ' . $id[$i] . '<br />Field Name:  View Angle <br />Old Value:  ' . $curr['name'] . '<br />New Value:  ' . $name[$i] . '<br />';
	} elseif ($curr['description'] != $description[$i]) {
		$query[$i] .= ' UPDATE ViewAngle SET description = \'' . $description[$i] . '\'  where name = \'' . $id[$i] . '\' ;';
		$message .= '<br />Record:  ' . $id[$i] . '<br />Field Name:  Description<br />Old Value:  ' . $curr['description'] . '<br />New Value:  ' . $description[$i] . '<br />';
	}

	if ($flag) {
		$viewCount = mysqli_fetch_array(runQuery('SELECT COUNT(*) AS viewCount FROM View WHERE viewAngle = \'' . $id[$i] . '\';'));
