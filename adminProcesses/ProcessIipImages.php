...
$missingSql = "select b.id, u.uin, i.originalFileName, i.imageType, '', i.imageWidth,
    ...
    join UserProperty up on (b.id = up.objectId and up.name='imageurl')
    where  $SELECT_LIMIT";
...
isMdb2Error ( $updateTypeStmt, $updateTypeSql );
...
while ( $row = $result->fetchRow () ) {
	...
	if ($message [0] == 'N') { // error in processing
		...
		if (empty ( $width ) || $width != $w || $height != $h) {
	...
	// update image type if necessary
	if ($newImageType != $imageType) {
		$params = array (
				$newImageType,
				$id 
		);
		$count = $updateTypeStmt->execute ( $params );
		isMdb2Error ( $count, "update image type id $id new type $newImageType sql: $updateSizeSql" );
		$message .= ": image type changed from $imageType to $newImageType ";
	}
}
echo "$message\n";
