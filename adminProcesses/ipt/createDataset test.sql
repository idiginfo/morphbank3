...
drop table IptIdsMbMisc;
		
create table IptIdsMbMisc (
	id int primary key
);
	
drop table IptOccurrenceMbMisc;


CREATE TABLE IptOccurrenceMbMisc (
...
drop table IptAudubonCoreMbMisc;


CREATE TABLE IptAudubonCoreMbMisc (
id int primary key, # morphbank id
occurrenceID varchar(100), # included in IPT
...
drop table IptResourceRelationshipMbMisc;


Create Table IptResourceRelationshipMbMisc(
...
#ids of specimens
insert into IptIdsMbMisc
select s.id from Specimen s join BaseObject o on o.id=s.id 
where basisofrecordid = 's' and institutionCode is not null and institutionCode !=' '
and institutionCode !='private'
and groupid in (select groupid from GroupsMbMisc);
 
drop table GroupsMbMisc;
		
truncate Table IptOccurrenceMbMisc; 

insert into IptOccurrenceMbMisc 
( 
id,
occurrenceID,
type,
...
left join ExternalLinkObject eo 
		on (eo.mbid=s.id and eo.description = 'dwc:occurrenceID')
left join Country c on l.country=c.description
where b.dateToPublish <= now() and s.basisOfRecordId = 'S'
;
		
update IptOccurrenceMbMisc s join ExternalLinkObject eo 
		on s.id=eo.mbid and eo.decription='identifier'
		set occurrenceId = eo.externalId where occurrenceId is null;


truncate table IptAudubonCoreMbMisc;

 insert into IptAudubonCoreMbMisc
(
id, 
occurrenceID,
identifier,
