...
function showUserLogo($userId) {
  ...
  if (!empty($userLogoArray['userLogo'])) {
    echo '<div class="logothumbnail">';
    echo '<a href="' . $userLogoArray['logoURL'] . '">';
    echo '<img src="' . $userLogoArray['userLogo'] . '" alt="logo" /></a></div>';
  } else {
    echo '&nbsp;';
  }
}
...
function showTsnData($tsnId) {
  ...
  for ($i = 0; $i < $arraySize; $i++) { // skip life node
    if ($taxonRankIdToDisplay[$arrayOfParents[$i]['rank_id']]['display']) {
      ...
      $hrefTsnName = isset($_GET['pop']) ? ('javascript:loadInOpener(\'' . $config->domain . 'Browse/ByImage/?tsn=' . $arrayOfParents[$i]['tsn'] . '\')') : ($config->domain . 'Browse/ByImage/?tsn=' . $arrayOfParents[$i]['tsn']);

      $hrefTsnTree = isset($_GET['pop']) ? ('javascript:loadInOpener(\'' . $browseByTaxonHref . '?tsn=' . $arrayOfParents[$i]['tsn'] . '\')') : ($browseByTaxonHref . '?tsn=' . $arrayOfParents[$i]['tsn']);
...
function showLocality($localityId) {
  ...
  $localityTableFields = array(
      ...
      array('field3' => 'latitude', 'field4' => 'Latitude: ', 'width' => 10, 'display' => false),
      array('field3' => 'longitude', 'field4' => 'Longitude: ', 'width' => 10, 'display' => false),
  ...
  for ($i = 0; $i < $arraySize; $i++) {
    if ($localityTableFields[$i]['display']) {
      echo '<tr>
        <th>' . $localityTableFields[$i]['label'] . '</th>
        <td align="left" width="65%">';
      $formatOutput = false;
      // formating ids like [id] - name
      if ($localityRecord[$localityTableFields[$i]['relatedTable']]) {
        $formatOutput = true;
        echo '[' . $localityRecord[$localityTableFields[$i]['field']] . '] - ' . $localityRecord[substr($localityTableFields[$i]['field'], 0, -4)];
      }
      ...
      if (!$formatOutput) {
        if ($localityTableFields[$i]['field'] == 'id') {
          echo '<a href="' . $popUrl . $localityId . '">' . $localityRecord[$localityTableFields[$i]['field']] . '</a>';
        } else {
  ...
  echo '<tr>
    <th><strong> Latitude:</strong></th>
		<td align="left" width="65%">' . truncateValue($localityRecord[$localityTableFields[7]['field3']]) . '</td>';
  echo "</tr>\n";
  echo '<tr>
    <th><strong> Longitude:</strong></th>
		<td align="left" width="65%">' . truncateValue($localityRecord[$localityTableFields[8]['field3']]) . '</td>';
  echo "</tr>\n";
  // Elevation
  echo '<tr>
    <th><strong> Elevation (m):</strong></th>
		<td align="left" width="65%">' . $localityRecord[$localityTableFields[10]['field']];
    if (($localityRecord[$localityTableFields[11]['field']] != $localityRecord[$localityTableFields[10]['field']])
          && ($localityRecord[$localityTableFields[11]['field']] != null))
    echo '-' . $localityRecord[$localityTableFields[11]['field']];
  echo '<td></tr>';
  echo '<tr>
    <th><strong> Depth (m):</strong></th>
		<td align="left" width="65%">' . $localityRecord[$localityTableFields[12]['field']];
    if (($localityRecord[$localityTableFields[13]['field']] != $localityRecord[$localityTableFields[12]['field']])
          && ($localityRecord[$localityTableFields[13]['field']] != null))
    echo '-' . $localityRecord[$localityTableFields[13]['field']];
  echo '<td></tr>';
  echo "</td></tr>\n</table>";
}
...
function getContributorSelectTag($selectedUser = null, $groupId = null) {
  ...
  if (empty($selectedUser))
    $selectedUser = $objInfo->getUserId();
  if (empty($groupId))
