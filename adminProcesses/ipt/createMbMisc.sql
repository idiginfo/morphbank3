Use MB32;
drop table IPT_DB.MbMiscId;
		
create table IPT_DB.MbMiscId (
	id int primary key
);
	
drop table IPT_DB.MbMiscOcc ;


CREATE TABLE IPT_DB.MbMiscOcc  (
...
drop table IPT_DB.MbMiscAC;


CREATE TABLE IPT_DB.MbMiscAC (
id int primary key, # morphbank id
occurrenceID varchar(100), # included in IPT
...
drop table IPT_DB.MbMiscRR;


Create Table IPT_DB.MbMiscRR (
...
# list of groups
create table IPT_DB.MbMiscGroups 
...
truncate Table IPT_DB.MbMiscId; 

#ids of specimens
insert into IPT_DB.MbMiscId
select s.id from Specimen s join BaseObject o on o.id=s.id 
where basisofrecordid = 's' and institutionCode is not null and institutionCode !=' '
and institutionCode !='private'
and groupid in (select groupid from IPT_DB.MbMiscGroups);
 
drop table IPT_DB.MbMiscGroups;
		
truncate Table IPT_DB.MbMiscOcc; 

insert into IPT_DB.MbMiscOcc 
( 
id,
occurrenceID,
type,
...
update IPT_DB.MbMiscOcc o join Specimen s on s.id=o.id
...
truncate table IPT_DB.MbMiscAC;

 insert into IPT_DB.MbMiscAC (
id, 
occurrenceID,
identifier,
type,
...
from Image i 
join IPT_DB.MbMiscOcc s on (i.specimenid=s.id)
