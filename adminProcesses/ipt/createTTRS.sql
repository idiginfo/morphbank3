Use MB32;
drop table IPT_DB.TTRSId;
		
create table IPT_DB.TTRSId (
	id int primary key
);
	
drop table IPT_DB.TTRSOcc ;


CREATE TABLE IPT_DB.TTRSOcc  (
...
drop table IPT_DB.TTRSAC;


CREATE TABLE IPT_DB.TTRSAC (
id int primary key, # morphbank id
occurrenceID varchar(100), # included in IPT
...
drop table IPT_DB.TTRSRR;


Create Table IPT_DB.TTRSRR (
...
truncate Table IPT_DB.TTRSId; 

#ids of specimens
insert into IPT_DB.TTRSId
...
update IPT_DB.TTRSOcc o join Specimen s on s.id=o.id
...
truncate table IPT_DB.TTRSAC;

 insert into IPT_DB.TTRSAC (
id, 
occurrenceID,
identifier,
type,
...
from Image i 
join IPT_DB.TTRSOcc s on (i.specimenid=s.id)
...
truncate IPT_DB.TTRSRR;

insert into IPT_DB.TTRSRR
