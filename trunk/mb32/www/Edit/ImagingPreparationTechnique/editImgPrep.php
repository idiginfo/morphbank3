<?php
  /**
   ...
   This script has only one function editImgPrep that displays the GUI and form data from the database.
   **/
  
  function editImgPrep()
  {
      ...
      $totalNumRows = mysqli_num_rows(runQuery("SELECT name AS Preparation, description 
      AS Description FROM ImagingPreparationTechnique ORDER BY name;"));
      
      $result = runQuery('SELECT name AS Preparation, description AS Description FROM ImagingPreparationTechnique ORDER BY name LIMIT ' . $offset . ', 20;');
      $numRows = mysqli_num_rows($result);
      
      echo '<form  name = "editPrep" method = "post" action = "modifyImgPrep.php" onsubmit = "return checkall()">
    <h1><b>Edit Imaging Preparation Technique</b></h1>
          <br /><br /><br />';
      ...
      <input type="hidden" name="flag" value="false" />
          <table width="600"> <tr><td><b>Imaging Preparation Technique <span class="req">*</span></b></td>
      ...
      <tr><td><br /> </td></tr>';
          ...
          for ($i = 0; $i < $numRows; $i++) {
              ...
              foreach ($row as $column => $val) {
                  ...
                  if ($column == "Preparation") {
                      echo '<td><input type="hidden" name="id' . $i . '" value="' . $val . '" />
                                        <input type="text" size ="55" name="' . $column . $i . '" value="' . $val . '" onchange = "changed()" /></td>';
                  } else
                      echo '<td><input type="text" size ="65" name="' . $column . $i . '" value="' . $val . '" onchange = "changed()" /> </td>';
              }
              echo '</tr>';
          }
      } else {
