...
function showUsers($id, $gmId, $rows, $current = true) {
  ...
  if (!empty($rows)) {
    ...
    foreach ($rows as $row) {
      ...
      $disabled = ($row['id'] + 1 == $id) ? 'disabled' : ''; // If this is the group created during sign up, do not allow modifation
      echo '<tr><td valign="middle">' . "\n";
      echo '<input type="checkbox" name="user[' . $count . '][user]" ' . $checked . ' value="' . $row['id'] . '" '.$disabled.' />';
      echo '&nbsp;&nbsp;' . $row['name'];
...
function buildRoleSelect($id, $gmId, $userId, $role, $count) {
  ...
  $disabled = ($userId + 1 == $id) ? 'disabled="disabled"' : '';
  ...
  if ($role == 'reviewer') {
    ...
    $html = '<select class="duplicate" name="user[' . $count . '][usergrouprole]" '.$disabled.'>';
