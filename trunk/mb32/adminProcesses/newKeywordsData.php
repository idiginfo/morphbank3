...
." left join CollectionObjects CO ON C.id = CO.collectionId";
	...
	switch ($objectTypeId) {
		...
		case "View":
			if ($SqlViewQuery == null) {
				...
				echo "View: SELECT $viewFields FROM $viewJoin where B.id = ?\n";
			}
			return $SqlViewQuery;
		case "Locality":
...
function prepareKeywordQueries(){
	...
	$keywordsTempStmt = $db->prepare($keywordsTempSql,$keywordsTempParams, MDB2_PREPARE_MANIP);
	...
	$keywordsInsertSql = "INSERT INTO Keywords(id, userId, groupId, dateToPublish, objectTypeId,"
		...
		." keywords, xmlKeywords, submittedBy, dateCreated"
		." FROM BaseObject WHERE id in (select id from KeywordsTemp)";

	$updateTaxaSql = "UPDATE Taxa SET keywords = ? WHERE id = ?";
	$param_types = array('text','integer');
	$taxaUpdateStmt = $db->prepare($updateTaxaSql,$param_types, MDB2_PREPARE_MANIP);
