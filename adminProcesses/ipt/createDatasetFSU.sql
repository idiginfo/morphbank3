...
truncate Table IPT_DB.FSUId; 

#ids of specimens
insert into IPT_DB.FSUId
select s.id from Specimen s join BaseObject b on b.id=s.id 
where basisofrecordid = 's' 
and b.groupId=475742;
 
#drop table ;
		
truncate Table IPT_DB.FSUOcc; 

insert into IPT_DB.FSUOcc 
( 
id,
occurrenceID,
type,
...
update IPT_DB.FSUOcc o join Specimen s on s.id=o.id
...
truncate table IPT_DB.FSUAC;

 insert into IPT_DB.FSUAC (
id, 
occurrenceID,
identifier,
type,
...
from Image i 
join IPT_DB.FSUOcc s on (i.specimenid=s.id)
...
truncate IPT_DB.FSURR;

insert into IPT_DB.FSURR
